...
Guacamole.Keyboard = function Keyboard(element) {
  ...
  var KeydownEvent = function (keyCode, keyIdentifier, key, location) {
    ...
    this.keysym =
      keysym_from_key_identifier(key, location) || keysym_from_keycode(keyCode, location);
  ...
  var KeyupEvent = function (keyCode, keyIdentifier, key, location) {
    ...
    // If key is known from keyCode or DOM3 alone, use that
    this.keysym =
      keysym_from_keycode(keyCode, location) || keysym_from_key_identifier(key, location); // keyCode is still more reliable for keyup when dead keys are in use
    ...
    if (!guac_keyboard.pressed[this.keysym]) this.keysym = recentKeysym[keyCode] || this.keysym;
