...
import CloudInstancesTableRow from './CloudInstancesTableRow';

interface ICloudInstancesTable {
  tableInstance: any;
  loading: boolean;
  filterInput: string;
...
const CloudInstancesTable = ({
  ...
  createType,
  createLink
}: ICloudInstancesTable) => {
  return (
    <>
      {loading ? (
        <Loader />
      ) : (
        <div className="flex flex-col w-[90%] rounded shadow-md dark:border dark:border-layer-600">
          <div className="flex items-center justify-between p-4 rounded-t bg-light-200 dark:bg-layer-200 border-b dark:border-layer-600">
          ...
          >
            ...
            <tbody {...tableInstance.getTableBodyProps()} className="font-normal">
              {tableInstance.rows.map((row) => {
                ...
                return <CloudInstancesTableRow key={row.id} row={row} />;
              })}
            </tbody>
          </table>
        </div>
...
export default CloudInstancesTable;
